...
public class Auth0AuthHandler
{
    ...
    public async Task<AuthResult> LoginWithDeviceCodeAsync()
    {
        try
        {
            ...
            Console.WriteLine(
                $"To sign in, use a web browser to open the page {deviceAuthorizationResponse.VerificationUriComplete}");
            ...
            return await PollForTokenAsync(deviceAuthorizationResponse.DeviceCode,
    ...
    private async Task<AuthResult> PollForTokenAsync(string deviceCode, int interval)
    {
        ...
        if (!string.IsNullOrEmpty(_clientSecret)) requestData.Add("client_secret", _clientSecret);

        var content = new FormUrlEncodedContent(requestData);

        var pending = true;
        var timeout = DateTime.Now.AddMinutes(10); // 10 min timeout
